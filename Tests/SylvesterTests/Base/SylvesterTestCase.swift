...
//            case docSupportInputsMainSyntaxMapJSON = "doc-support-inputs-main-syntax-map.json"
    ...
    static var testFixturesURL: URL = .init(fileURLWithPath: "/Volumes/Repositories/Private/Fork/Library/Sylvester/Tests/SylvesterTests/Fixtures", isDirectory: true)

    static var testProjectDirectoryPath: String = filePath(for: TestFixture.Directory.testProjectDirectory).path

    static var testModule: Module = {
        Module(xcodeBuildArguments: [], name: nil, inPath: testProjectDirectoryPath)!
    }()
    ...
    ///
    static var testModuleSourceFiles: [String: String] = Dictionary(
        uniqueKeysWithValues: SourceFile.allCases.map(\.rawValue)
            .map { ($0, testFixturesURL.appendingPathComponent(TestFixture.Directory.testProjectDirectory.rawValue).appendingPathComponent("Test").appendingPathComponent("\($0).swift").path) }
    )
...
//                sourceFile = .docSupportInputsMain
    ...
    private func writeJSONFixture<
        T1: Encodable,
        T2: RawRepresentable
    >(
        for encodableObject: T1,
        enumCase: T2,
    ...
    func diff(_ string: String, expected expectedString: String, withNewLines: Bool = true) -> String? {
        ...
        case .differenceAtIndex:
            return prettyDescriptionOfFirstDifferenceResult(
                firstDifferenceResult: firstDifferenceResult,
                string1: string,
    ...
    func SylvesterAssert<T: Encodable>(
        _ object: T,
