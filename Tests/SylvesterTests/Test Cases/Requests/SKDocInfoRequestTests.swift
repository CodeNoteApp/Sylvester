...
class SKDocInfoRequestTests: SylvesterTestCase {
    ...
    class MethodIterator<Entity: SKSequence>: SKPreOrderDFSIterator<Entity> {
        override func next() -> Entity? {
            guard let nextSubstructure = super.next() as? CustomEntity else { return nil }
            if nextSubstructure.kind == .functionMethodInstance {
                return nextSubstructure as? Entity
            } else {
                return next()
            }
        }
