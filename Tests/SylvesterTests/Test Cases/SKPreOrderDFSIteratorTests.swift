...
class SKPreOrderDFSIteratorTests: SylvesterMockEditorOpenTestCase {
    ...
    class FunctionIterator<Substructure: SKSequence>: SKPreOrderDFSIterator<Substructure> {
        override func next() -> Substructure? {
            guard let nextSubstructure = super.next() as? CustomSubstructure
            else { return nil }
            if nextSubstructure.isFunction {
                return nextSubstructure as? Substructure
            } else {
                return next()
            }
        }
    ...
    final class CustomSubstructure: SKBaseSubstructure, SKFinalSubclass {
        override class func iteratorClass<T>() -> SKPreOrderDFSIterator<T>.Type where T : SKSequence {
    ...
    func testViewControllerEditorOpenOrder() {
        ...
        for (index, substructure) in viewControllerEditorOpenResponse.topLevelSubstructures.enumerated() {
            ...
            XCTAssertLessThan(
                index,
                preOrderDFSViewControllerEditorOpenNames.count,
                "response contains more substructures than the test expects"
            )
            XCTAssertEqual(
                substructure.name,
                preOrderDFSViewControllerEditorOpenNames[index],
    ...
    func testViewControllerDocInfoOrder() {
        ...
        for (index, entity) in viewControllerDocInfoResponse.topLevelEntities!.enumerated() {
            ...
            XCTAssertLessThan(
                index,
                preOrderDFSViewControllerDocInfoNames.count,
                "the response contains more entities than the test expects"
            )
            XCTAssertEqual(
                entity.name,
                preOrderDFSViewControllerDocInfoNames[index],
                "`SKPreOrderDFSIterator` is not pre-order DFS"
            )
            count += 1
        }
    ...
    func testSubstructureSubclassIterator() throws {
        ...
        let functionCount = customEditorOpen.topLevelSubstructures.reduce(0) { count, _ in count + 1 }
