...
open class SKCodeCompletionSession {
    // MARK: - Static Properties

    static let serialRequestQueue = DispatchQueue(
        label: Constants.Identifier.codeCompletionSessionQueue,
        qos: .userInitiated,
    ...
    public convenience init(
        ...
        self.init(
            file: File(pathDeferringReading: filePath),
            offset: offset,
    ...
    public func open() {
        ...
        SKCodeCompletionSession.serialRequestQueue.async { [weak self] in
            ...
            do {
                ...
                handleError(
                    error.toSKError(),
                    for: Response(kind: .open, options: options, codeCompletion: nil)
                )
            }
        }
    }
    ...
    public func update() {
        ...
        SKCodeCompletionSession.serialRequestQueue.async { [weak self] in
            ...
            do {
                let response = try SylvesterInterface.shared.codeCompletionUpdate(
                    file: file,
                    offset: offset,
                    options: options
                )
    ...
    func close(shouldDispatchAsynchronous: Bool) {
        ...
        let sendCloseRequest: (SKCodeCompletionSession) -> Void = { session in
            do {
                let response = try SylvesterInterface.shared.codeCompletionClose(
